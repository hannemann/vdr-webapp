...
function Transcoder(source) {
	...
	Transcoder.prototype._exec = function(a) {
		...
		console.log('Spawning avconv ' + a.join(' '));
		
		this.child = spawn('avconv', a, {
			cwd: os.tmpdir()
		});
		this._parseMetadata(this.child);
        ...
        this.child.on('exit', function(code) {
			...
			else self.emit('error', new Error('Avconv error: ' + self.lastErrorLine));
		});
...
//		if (format.toLowerCase() == 'mp4') this.args['movflags'] = [ '-movflags', 'frag_keyframe+faststart' ];
    ...
    /**
     * create canvas and center video
     * @param width
     * @param height
     * @returns {Transcoder}
     */
    ...
    Transcoder.prototype.quantMin = function (min) {

        this.args["quantization"] = [ '-qmin', min ];

        return this;
    };

    Transcoder.prototype.quantMax = function (max) {

        this.args["quantization"] = [ '-qmax', max ];

        return this;
    };

    Transcoder.prototype.quality = function (crf) {

        this.args["quality"] = [ '-crf', crf ];
